...
- **Sensitive settings (passphrase `sassyloveslarry`).** Behind a deliberately quiet "Advanced" row at the bottom of that panel. It shifts the sheet's **planned schedule** — all 36 planned start/finish times move by one offset, so every leg keeps its planned duration and the team's pace work survives. Runners, miles, recorded times, and the tracker's own columns are untouched, and the backend refuses the shift once any leg is recorded. Useful for a full-dress drill before race week: point the plan at today and every ETA, planned finish, and delta reads like the real thing.
  - **This is obfuscation, not security.** The passphrase sits in `index.html` and in `Code.gs`, so anyone who can open the page can read it. Its job is to stop a mis-tap from rewriting 72 cells, not to keep anyone out. Auth is still the URL plus the team key.
  - **Needs a backend redeploy.** `shiftPlan_` is new in `Code.gs`; re-paste it into Apps Script and redeploy before the button will do anything.
