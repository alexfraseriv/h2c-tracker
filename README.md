...
- **You always know what got logged.** Tap the status pill (or History) for a per-change log: `↻ queued` → `✓ Confirmed in sheet 3:48:51 AM`, or `✗ failed: <actual error> — will keep retrying`, or `⚠ conflict — sheet shows a different time, kept the sheet value`. Sync retries with backoff automatically; "Sync now" forces it.
...
- **Roadkills 🪦.** On any completed leg: slider + steppers, "Save roadkills." Counts go to a separate **App Kills** tab (auto-created; the tracker tab is never touched), sync offline like everything else, show on the leg rows, and total up above the timeline.
- **One vocabulary.** "plan" = the sheet's schedule, "ETA" = live estimate from actuals so far (a legend sits above the timeline). Weekday shows only when a time isn't on race-start day.
